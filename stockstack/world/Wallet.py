...
async def putmoney(cid: int, amount: int | None = None):
    async with aiohttp.ClientSession() as session:
        async with session.put(f'http://stockwallet:{Settings.get()["stockwallet"]["web"]["port"]}/{cid}',
                               data=
                               ('{"user_id": ' + str(cid) + (
...
async def deltamoney(cid: int, amount: int):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'http://stockwallet:{Settings.get()["stockwallet"]["web"]["port"]}/{cid}',
...
async def getmoney(cid: int):
    async with aiohttp.ClientSession() as session:
        async with session.get(
                f'http://stockwallet:{Settings.get()["stockwallet"]["web"]["port"]}/{cid}') as resp:
            j = await resp.json()
            return j['amount']
